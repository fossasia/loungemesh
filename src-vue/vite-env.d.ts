...
interface ImportMetaEnv {
  ...
  /**
   * URL of the eventyay-loungemesh plugin token exchange endpoint.
   * Example: https://eventyay.com/api/v1/loungemesh/token/
   * When set, /join/:id requires a valid opaque token in the URL query.
   */
  readonly VITE_EVENTYAY_JWT_ENDPOINT?: string;
  /**
   * Comma-separated list of origins allowed to embed LoungeMesh in an iframe.
  ...
  /** Set to `true` to log video/audio pipeline events (filter console by loungemesh:media). */
