...
describe('chatMessage', () => {
  it('creates and edits messages with history', () => {
    ...
    expect(applyChatEdit([msg], 'missing', 'x', 1)).toEqual([msg]);
    const bare = { id: 'u1', text: 'hi', nr: 2, messageId: 'm2' };
    const editedBare = applyChatEdit([bare], 'm2', 'hi2', 2000);
    expect(editedBare[0].history).toEqual(['hi']);
    const edited = applyChatEdit([msg], 'm1', 'hello again', 1000);
    expect(edited[0].text).toBe('hello again');
    expect(edited[0].history).toEqual(['hello']);
    expect(applyChatEdit(edited, 'm1', 'hello again', 1000)).toBe(edited);
  });
  ...
  it('formats edited time and author styling', () => {
    ...
    expect(canEditChatMessage(createChatMessage('u1', 'x', 1), 'u1', false)).toBe(true);
    expect(canEditChatMessage(createChatMessage('u1', 'x', 1), 'u2', true)).toBe(true);
    expect(canEditChatMessage(createChatMessage('u1', 'x', 1), 'u2', false)).toBe(false);
    expect(canEditChatMessage(createChatMessage('u1', 'x', 1), '', false)).toBe(false);
  });
});
