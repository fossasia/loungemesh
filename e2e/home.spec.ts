...
test.describe('LoungeMesh', () => {
  test('home page loads with LoungeMesh branding', async ({ page }) => {
    const errors: string[] = [];
    page.on('pageerror', (e) => errors.push(e.message));

    await page.goto(baseURL);
    await expect(page).toHaveTitle(/LoungeMesh/i);
    await expect(page.getByRole('heading', { name: /^LoungeMesh$/i })).toBeVisible();
  ...
  test('enter lobby route resolves', async ({ page }) => {
    await page.goto(`${baseURL}/enter/loungemesh`);
